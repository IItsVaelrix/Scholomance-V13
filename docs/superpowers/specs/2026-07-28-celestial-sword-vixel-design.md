# CelestialSwordVixel — Vixel Pipeline Design Specification

**Date:** 2026-07-28  
**Asset Name:** `CelestialSwordVixel`  
...
Recreate the **Celestial Sword** (`0cd89f9a-b8d7-495d-aafd-6d4ca3140205.png`) as `CelestialSwordVixel` using the upgraded PolarisOS Vixel Pipeline.

The asset incorporates:
- Mathematically symmetrical \(32\times128\) SCDL asset assembly across central axis \(x=16.0\)
...
## 3. Structural Layer Assembly (`CelestialSwordVixel.scdl`)
...
## 4. Wand Vector Stroke Formulas (`CelestialSwordVixel.wand.json`)

1. **`CelestialSwordVixel.blade_contour` (`edge_trace`):** Golden edge filigree (`#D4AF37`).
2. **`CelestialSwordVixel.guardian_wings` (`edge_trace`):** Guardian crosswing contour (`#D4AF37`).
3. **`CelestialSwordVixel.guardian_eye` (`edge_trace`):** Guardian eye diamond outline (`#E6AA4E`).
4. **`CelestialSwordVixel.stellar_lattice` (`parametric_curve`):** Central cyan core lightning beam (`#80FFFF`).
5. **`CelestialSwordVixel.pommel_origin_ring` (`parametric_curve`):** Circular pommel ring (`#D4AF37`).
...
* **Pipeline Commands:**
  ```bash
  node codex/core/pixelbrain/scdl/scdl.cli.js compile PolarisOS/worldpacks/shrine-demo/scdl/CelestialSwordVixel.scdl --export json --out PolarisOS/worldpacks/shrine-demo/scdl/CelestialSwordVixel-json.json
  node scripts/wand-vixel-pipeline.mjs CelestialSwordVixel
  node PolarisOS/scripts/vixel-rasterize.mjs CelestialSwordVixel 4
  ```
