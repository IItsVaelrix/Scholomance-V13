...
## 7. Correspondence Health Protocol

Every entry in §2 makes machine-checkable claims. The `Evidence` field names
file paths and symbols; those names rot when the codebase moves underneath
them, and prose rots silently — a stale entry reads exactly like a fresh one.

This section specifies the gate that makes that rot loud. It is the closing
line of this document turned into a mechanism: when the bridge and the
territory disagree, the territory wins, and the gate is how the disagreement
gets reported rather than discovered years later.

**Status:** specified here, not yet implemented. No `verify:correspondence`
script exists at the time of writing.

That this rot is real in this repository is not hypothetical. On 2026-07-30,
`scripts/verify_capabilities.py` — which performs the same class of check over
`*.capability.json` packets — reported two live cases: `career-ats` names
`logProb`, which appears nowhere in `codex/core/career/ats-hmm/index.js`, and
names `requiresUserApproval`, which appears nowhere in
`src/lib/career/suggestions/apply-suggestions.ts`. Both files still exist, so a
path-only check passes. The packet is naming something that is gone.

### 7.1 What is checked, and what is not

| Field | Authored by | Covered by `evidence-sha` | Staleness detected via |
|-------|-------------|---------------------------|------------------------|
| `Evidence` paths | Human, machine-verifiable | Yes | `GONE` / `DRIFT` |
| `Evidence` symbols | Human, machine-verifiable | Yes | `GONE` / `DRIFT` |
| `Strength` | Human judgement | **No** | `strength-reviewed` date vs. `DRIFT` |
| `Preserved` | Human judgement | **No** | `strength-reviewed` date vs. `DRIFT` |
| `NOT preserved` | Human judgement | **No** | `strength-reviewed` date vs. `DRIFT` |
| `Math`, `Directionality`, `Notes` | Human prose | **No** | Not gated |

The split is the point. The sha covers only what a machine can falsify. The
grade is never hashed, because hashing a human judgement would make the gate
compare a generator against its own output.

### 7.2 The evidence projection

`evidence-sha` is SCD64 (§SCR-002 classifies checksum ↔ content-addressed hash
as `ID`) over a canonical projection of the `Evidence` field alone:

```
projection = {
  paths:   sorted, deduped, repo-relative POSIX paths
  symbols: sorted, deduped identifiers
}
evidence-sha = scd64(canonical_json(projection))
```

Normalization rules, all required for determinism:

1. **Bare filenames resolve to repo-relative paths.** SCR-002 cites
   `vri-compiler.js` and `construction-schema.js` without directories; these
   resolve to `codex/core/pixelbrain/vixel/vri-compiler.js` and
   `codex/core/pixelbrain/construction/construction-schema.js`.
2. **A bare name resolving to more than one path is an authoring error, not a
   tie to break.** SCR-002's `scd64/` matches both `src/core/scd64` and
   `src/diagnostics/scd64`. Ambiguity must be reported as `AMBIGUOUS` and fixed
   in the entry, never silently resolved to the first match — a projection that
   guesses is not deterministic.
3. **Prose contributes nothing.** Only backticked identifiers and paths are
   extracted. Reformatting a sentence must not perturb the sha.
4. **Symbol extraction must not match English prose.** See §7.5.

Consequence: an internal refactor that preserves the cited surface is silent. A
rename, a deletion, or a moved file is loud. The gate fires if and only if a
claim this document makes became false.

### 7.3 Verdicts

| Verdict | Meaning | Action |
|---------|---------|--------|
| `OK` | Every cited path and symbol resolves; `evidence-sha` matches. | None. |
| `GONE` | A cited path or symbol no longer exists. The entry is factually false. | Fix the entry. Fail. |
| `DRIFT` | Everything resolves, but `evidence-sha` differs from the recorded value and `strength-reviewed` predates the change. | Re-read the entry's `Strength` against current code, then re-date. Fail. |
| `AMBIGUOUS` | A bare name resolves to more than one path (§7.2 rule 2). | Disambiguate the citation. Fail. |

`DRIFT` is not "the file changed." It is *"the evidence this grade rests on
changed, and nobody has re-read the grade since."* That is the only claim the
tool is entitled to make, because the tool cannot evaluate a correspondence.

### 7.4 Why this gate can fail

A gate that generates a claim from the code and then checks that claim against
the same code cannot fail; it tests only whether someone re-ran the generator.
This protocol avoids that by construction:

- The machine supplies the **facts** — do these paths and symbols exist, and
  have they changed.
- The human supplies the **grade** — `ID`, `SC`, `FA`, `MT`, `FF` — which no
  script can derive, because it is a claim about structure preservation.
- The gate's output is never a grade. It is a demand that a human re-read one.

`DRIFT` is therefore the mechanical trigger for the demotion rule already
specified in §1: *an entry may be demoted at any time a counterexample is
found.* The gate does not find counterexamples; it finds the moments when one
became likely and a human stopped looking.

### 7.5 Preconditions

1. **The symbol extractor must not match English prose.** The extractor in
   `scripts/verify_capabilities.py` currently does, in two distinct ways, and
   both must be fixed before that code is reused here. Its `_SYMBOL_PATTERNS`
   entry `\b[A-Za-z][a-z0-9]*[A-Z][A-Za-z0-9]*\b` is intended to require an
   internal capital (camelCase), but its `[a-z0-9]*` run may match zero
   characters, so any ALL-CAPS word qualifies — which is how the prose word
   `ONCE` became a reported missing symbol, defeating the deliberate
   underscore requirement in the neighbouring ALL-CAPS pattern. Its
   `\b[A-Z][a-z]{2,}\b` entry matches any capitalized word of three or more
   letters, which is how `Node` became one. The file's own comment states that
   "ordinary prose contributes no candidates at all"; measured against a single
   plain sentence, it produced nine.
2. **Report-only before hard-fail.** Seeding 25 entries at once will surface
   real rot immediately. The gate reports without failing until the initial
   backlog is cleared, then becomes blocking. A gate switched on while already
   red teaches everyone to ignore it, which is the failure mode
   `verify_capabilities.py` warns about in its own comments: *a false alarm
   trains people to ignore the gate, which is worse than a miss.*
3. **Coverage must be stated, never assumed.** An entry with no
   `evidence-sha` is unverified, not verified-clean. The summary in §3 must
   distinguish the two, for the same reason a probe that reports no findings
   has not proved absence.

### 7.6 Entry block format

Two fields are added to the §2 schema. Both are optional; absent means
unverified.
...
- Evidence:           ...
- Notes:              ...
- evidence-sha:       scd64:<digest>      # machine, over §7.2 projection
- strength-reviewed:  YYYY-MM-DD          # human, when the grade was last read
```

### 7.7 Relationship to the substrate

§6 gap 1 notes that these entries should be ingested as grounding anchors once
the substrate vector engine is online. `evidence-sha` is what makes that safe:
an entry whose evidence has drifted must not be served as corpus attestation
for a `concept-chemistry.js` reaction. Refusing to serve a drifted entry is the
same posture `verify_capabilities.py` takes when it refuses to load a packet
whose checksum does not match — the packet is not wrong, it is *unverifiable*,
and unverifiable claims must not become grounding.
