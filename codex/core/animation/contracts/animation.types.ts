...
export interface AnimationIntent {
  ...
  constraints?: {
    ...
    /** Safety mode for the aesthetic immune system (controls similarity dampening) */
    motionSafetyMode?: 'off' | 'warn-only' | 'dampen-soft' | 'dampen-hard' | 'reject';
  };
...
export interface ResolvedMotionOutput {
  ...
  /** Hexadecimal 2.5-bit / 4-bit TurboQuant quantized signature payload (self-describing) */
  quantizedSignature?: {
    data: string;
    norm: number;
    dimension: number;
    sampleCount: number;
    channels: readonly ['translateX', 'translateY', 'scale', 'opacity'];
    backend: 'js' | 'wasm';
  };

  /** Computed semantic similarity score against golden curves or active states */
  vectorSimilarity?: number;
  
  /** Closest matched aesthetic archetype name */
  nearestMotionArchetype?: 'fade-standard' | 'slide-smooth' | 'bounce-impact' | 'unknown';
...
export interface AnimationAmpConfig {
  ...
  /** The vector processor backend execution mode (js, wasm, or auto) */
  vectorBackend?: 'js' | 'wasm' | 'auto';

  /** Enable deterministic vector quantization and similarity checks (canonical JS runtime) */
  enableVectorQuantization: boolean;

  /** Dimensionality of sampled motion vectors (default: 256) */
  vectorDimension: number;

  /** Aesthetic immune system safety dampening mode */
  motionSafetyMode?: 'off' | 'warn-only' | 'dampen-soft' | 'dampen-hard' | 'reject';
}
...
// ─── Active Shared Configuration (Synchronous Bridge) ───────────────────────

export let activeAmpConfig: AnimationAmpConfig = { ...DEFAULT_AMP_CONFIG };

export function setActiveAmpConfig(cfg: AnimationAmpConfig): void {
  activeAmpConfig = { ...cfg };
}
...
export const AMP_ERROR_CODES = {
  ...
  AESTHETIC_VIOLATION: 'AMP-ERR-010',
} as const;
