...
const CLOSED_CLASS_MAP = Object.freeze({
  ...
  // Negation → NEGATED
  // `never` is negation + TIME_REF, not PAST: it quantifies over all time
  // rather than pointing behind us. It was listed twice — here as
  // ['NEGATED','PAST'] and again under the deictic block as
  // ['NEGATED','TIME_REF'] — and the later entry silently won, so TIME_REF is
  // what the engine has always actually returned. Kept, and now stated once.
  not: ['NEGATED'], no: ['NEGATED'], never: ['NEGATED', 'TIME_REF'],
