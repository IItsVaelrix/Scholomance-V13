...
export function parseBytecodeToFormula(bytecode) {
  ...
  try {
    ...
    const typeMap = {
      ...
      'V': FORMULA_TYPES.VECTORIZED_TEXT,
      'B': FORMULA_TYPES.FIBONACCI,
    };
    ...
    const coordinateFormula =
      type === FORMULA_TYPES.VECTORIZED_TEXT
        ? {
            type,
            text: '',
            fontSize: 24,
        ...
        : {
            ...
            parameters: type === FORMULA_TYPES.PARAMETRIC_CURVE
              ? { n: 64, a: w || 40, b: 0.1, cx: 80, cy: 72 }
              : {},
          };
...
export function formulaToBytecode(formula) {
  ...
  const typeCode = {
    ...
    [FORMULA_TYPES.VECTORIZED_TEXT]: 'V',
    [FORMULA_TYPES.FIBONACCI]: 'B',
  }[formulaType || ''] || 'X';
