...
func _get_importer_name() -> String:
	return "scholomance.pixelbrain.godot"

func _get_visible_name() -> String:
	return "Scholomance PixelBrain Artifact"

func _get_recognized_extensions() -> PackedStringArray:
	return PackedStringArray(["pbrain"])
...
func _import(source_file: String, save_path: String, _options: Dictionary, _platform_variants: Array[String], _gen_files: Array[String]) -> Error:
	...
	if not ArtifactLoader.is_kind(artifact, ArtifactLoader.PIXELBRAIN_KIND):
		if strict_validation:
			return ERR_INVALID_DATA
		return OK

	if not ArtifactLoader.validate_pixelbrain_artifact(artifact, strict_validation):
		return ERR_INVALID_DATA

	var renderer := PixelBrainRenderer.new()
	var scene := renderer.build_pixelbrain_scene(artifact)
