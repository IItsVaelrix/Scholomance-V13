...
const CONTRACT = 'PB-SEMANTIC-REACTOR-BENCHMARK-v1';
const OUTPUT_PATH = 'docs/superpowers/evidence/2026-08-11-semantic-fission-reactor-benchmark.json';
...
function deterministicSplit(tasks) {
  ...
  for (const relation of Object.keys(RELATIONS)) {
    ...
    test[relation] = ordered.slice(
      ...
      TRAIN_PER_RELATION + VALIDATION_PER_RELATION + TEST_PER_RELATION,
    );
  }
  return { training, validation, test };
}

function sampleFreshGloballyDisjoint(rows, perRelation, excludedSourceIds) {
...
function runBenchmark() {
  ...
  const pilotReservation = sampleTasks(
    rows,
    PRIOR_BLIND_PER_RELATION + TRAIN_PER_RELATION + VALIDATION_PER_RELATION + TEST_PER_RELATION,
  );
