import {
  COMBAT_CHEST_UNLOCK_DEFAULTS,
  COMBAT_CHEST_UNLOCK_SAMPLE,
  COMBAT_CHEST_UNLOCK_SETTINGS_KEY,
} from './gameChestUnlock.config.js';
...
function readSettings(storage) {
  if (!storage) return { ...COMBAT_CHEST_UNLOCK_DEFAULTS };
  const parsed = safeJsonParse(storage.getItem(COMBAT_CHEST_UNLOCK_SETTINGS_KEY), {});
  return {
    enabled: parsed.enabled ?? COMBAT_CHEST_UNLOCK_DEFAULTS.enabled,
    volume: clamp01(parsed.volume ?? COMBAT_CHEST_UNLOCK_DEFAULTS.volume),
  };
}
...
function writeSettings(storage, settings) {
  if (!storage) return;
  try {
    storage.setItem(COMBAT_CHEST_UNLOCK_SETTINGS_KEY, JSON.stringify(settings));
  } catch {
    // Ignore quota errors.
  }
}
...
export function createGameChestUnlockService(options = {}) {
  const storage = options.storage ?? (canUseBrowser() ? window.localStorage : null);
  const sampleUrl = options.sampleUrl || COMBAT_CHEST_UNLOCK_SAMPLE.url;
  ...
  async function playUnlockInternal() {
    if (!settings.enabled) {
      pendingPlay = false;
      return false;
    }
  ...
  function getState() {
    return {
      sampleId: COMBAT_CHEST_UNLOCK_SAMPLE.id,
      sampleTitle: COMBAT_CHEST_UNLOCK_SAMPLE.title,
      sampleUrl,
...
export function getGameChestUnlockService() {
  if (!singleton) {
    singleton = createGameChestUnlockService();
  }
  return singleton;
}

export function resetGameChestUnlockServiceForTests() {
  singleton?.destroy();
  singleton = null;
}