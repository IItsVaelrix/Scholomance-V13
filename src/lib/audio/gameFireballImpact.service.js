import {
  ...
  GAME_FIREBALL_IMPACT_SETTINGS_KEY,
} from './gameFireballImpact.config.js';
...
function readSettings(storage) {
  if (!storage) return { ...GAME_FIREBALL_IMPACT_DEFAULTS };
  const parsed = safeJsonParse(storage.getItem(GAME_FIREBALL_IMPACT_SETTINGS_KEY), {});
  return {
    enabled: parsed.enabled ?? GAME_FIREBALL_IMPACT_DEFAULTS.enabled,
    volume: clamp01(parsed.volume ?? GAME_FIREBALL_IMPACT_DEFAULTS.volume),
  };
}
...
function writeSettings(storage, settings) {
  if (!storage) return;
  try {
    storage.setItem(GAME_FIREBALL_IMPACT_SETTINGS_KEY, JSON.stringify(settings));
  } catch {
    // Ignore quota errors.
  }
}
...
export function createGameFireballImpactService(options = {}) {
  const storage = options.storage ?? (canUseBrowser() ? window.localStorage : null);
  const sampleUrl = options.sampleUrl || GAME_FIREBALL_IMPACT_SAMPLE.url;
  ...
  function getState() {
    return {
      sampleId: GAME_FIREBALL_IMPACT_SAMPLE.id,
      sampleTitle: GAME_FIREBALL_IMPACT_SAMPLE.title,
      sampleUrl,
...
export function getGameFireballImpactService() {
  if (!singleton) {
    singleton = createGameFireballImpactService();
  }
  return singleton;
}

export function resetGameFireballImpactServiceForTests() {
  singleton?.destroy();
  singleton = null;
}