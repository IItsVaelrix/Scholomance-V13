import {
  GAME_ICE_SPELL_IMPACT_DEFAULTS,
  GAME_ICE_SPELL_IMPACT_SAMPLE,
  GAME_ICE_SPELL_IMPACT_SETTINGS_KEY,
} from './gameIceSpellImpact.config.js';
...
function readSettings(storage) {
  if (!storage) return { ...GAME_ICE_SPELL_IMPACT_DEFAULTS };
  const parsed = safeJsonParse(storage.getItem(GAME_ICE_SPELL_IMPACT_SETTINGS_KEY), {});
  return {
    enabled: parsed.enabled ?? GAME_ICE_SPELL_IMPACT_DEFAULTS.enabled,
    volume: clamp01(parsed.volume ?? GAME_ICE_SPELL_IMPACT_DEFAULTS.volume),
  };
}
...
function writeSettings(storage, settings) {
  if (!storage) return;
  try {
    storage.setItem(GAME_ICE_SPELL_IMPACT_SETTINGS_KEY, JSON.stringify(settings));
  } catch {
    // Ignore quota errors.
  }
}
...
export function createGameIceSpellImpactService(options = {}) {
  const storage = options.storage ?? (canUseBrowser() ? window.localStorage : null);
  const sampleUrl = options.sampleUrl || GAME_ICE_SPELL_IMPACT_SAMPLE.url;
  ...
  async function playImpactInternal() {
    if (!settings.enabled) {
      pendingPlay = false;
      return false;
    }
  ...
  function getState() {
    return {
      sampleId: GAME_ICE_SPELL_IMPACT_SAMPLE.id,
      sampleTitle: GAME_ICE_SPELL_IMPACT_SAMPLE.title,
      sampleUrl,
...
export function getGameIceSpellImpactService() {
  if (!singleton) {
    singleton = createGameIceSpellImpactService();
  }
  return singleton;
}

export function resetGameIceSpellImpactServiceForTests() {
  singleton?.destroy();
  singleton = null;
}